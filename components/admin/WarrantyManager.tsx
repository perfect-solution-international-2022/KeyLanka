...
export function WarrantyManager({ initialWarranties, initialPolicies }: { initialWarranties: Warranty[]; initialPolicies: Policy[] }) {
  ...
  return <div className="space-y-8">
    <section className="rounded-xl border bg-card p-6"><h2 className="mb-1 font-semibold">Warranty Types</h2><p className="mb-5 text-sm text-muted-foreground">No Warranty is always available free; add paid or free warranty options here.</p>
      <form onSubmit={add} className="mb-5 grid gap-3 sm:grid-cols-[1fr_140px_160px_auto]"><Input required placeholder="Warranty name" value={draft.name} onChange={e=>setDraft({...draft,name:e.target.value})}/><Input required type="number" min="0" placeholder="Days" value={draft.days} onChange={e=>setDraft({...draft,days:e.target.value})}/><Input required type="number" min="0" step="0.01" placeholder="Price" value={draft.price} onChange={e=>setDraft({...draft,price:e.target.value})}/><button className="rounded-lg bg-brand px-4 text-sm font-semibold text-white">Add</button></form>
      <div className="space-y-3">{warranties.map((w,i)=><div key={w.id} className="grid gap-3 rounded-lg border p-3 sm:grid-cols-[1fr_120px_150px_auto_auto]"><Input value={w.name} onChange={e=>setWarranties(v=>v.map((x,j)=>j===i?{...x,name:e.target.value}:x))}/><Input type="number" min="0" value={w.days} onChange={e=>setWarranties(v=>v.map((x,j)=>j===i?{...x,days:Number(e.target.value)}:x))}/><Input type="number" min="0" value={w.price} onChange={e=>setWarranties(v=>v.map((x,j)=>j===i?{...x,price:e.target.value}:x))}/><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={w.active} onChange={e=>setWarranties(v=>v.map((x,j)=>j===i?{...x,active:e.target.checked}:x))}/>Active</label><button onClick={()=>saveWarranty(w)} className="rounded border px-3 text-sm">Save</button></div>)}</div>
    </section>
