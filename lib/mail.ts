  ...
  return layout(
    ...
    `
      ...
      <p style="margin: 24px 0;">
        <a href="${resetUrl}" style="background:#e8332a; color:#fff; text-decoration:none; padding: 12px 24px; border-radius: 6px; font-weight: bold; display:inline-block;">Reset Password</a>
      </p>
...
export function renderOrderConfirmationEmail(params: {
  ...
  const rows = params.items
    .map(
      (i) => `
        <tr>
          <td style="padding:8px 0; border-bottom:1px solid #f3f4f6;">${i.name} &times; ${i.quantity}</td>
  ...
  return layout(
    ...
    `
      <p>Hi ${params.name},</p>
      <p>Thanks for your order! We've received <strong>${params.orderNumber}</strong> and it's now being processed.</p>
      ...
      <p style="font-size: 13px; color:#6b7280;">Shipping to: ${params.shippingAddress}</p>
    `
  );
}
